...
if st.sidebar.button("백테스트 실행"):
    with st.spinner("백테스트 실행 중..."):
        ...
        if df is not None and len(df) > 1:
            ...
            # 누적 수익률 및 드로우다운 차트 (subplot)
            fig1 = make_subplots(
                rows=2, cols=1,
                subplot_titles=('누적 수익률 (Cumulative Return)', '드로우다운 (Drawdown)'),
                vertical_spacing=0.1
            )
            
            # 1. 누적 수익률 계산 및 차트
            strategy_returns = [(value / initial_balance - 1) * 100 for value in balance_df['total_value']]
            
            # 매수후보유 누적 수익률
            benchmark_returns = []
            for _, row in balance_df.iterrows():
                benchmark_return = (row['price'] / df.iloc[0]['close'] - 1) * 100
            ...
            # 2. 드로우다운 계산 및 차트
            # Strategy 드로우다운
            strategy_cummax = pd.Series(balance_df['total_value']).cummax()
            strategy_drawdown = ((pd.Series(balance_df['total_value']) - strategy_cummax) / strategy_cummax * 100).tolist()
            strategy_mdd_value = min(strategy_drawdown)
            strategy_mdd_line = [strategy_mdd_value] * len(balance_df)
            
            # Buy & Hold 드로우다운
            benchmark_values = [initial_balance * (row['price'] / df.iloc[0]['close']) for _, row in balance_df.iterrows()]
            benchmark_cummax = pd.Series(benchmark_values).cummax()
            benchmark_drawdown = ((pd.Series(benchmark_values) - benchmark_cummax) / benchmark_cummax * 100).tolist()
            benchmark_mdd_value = min(benchmark_drawdown)
            benchmark_mdd_line = [benchmark_mdd_value] * len(balance_df)
            ...
            # Y축 레이블 설정
            fig1.update_yaxes(title_text="수익률 (%)", row=1, col=1)
            fig1.update_yaxes(title_text="드로우다운 (%)", row=2, col=1)
            fig1.update_xaxes(title_text="시간", row=2, col=1)
